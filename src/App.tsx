...
import { Monitor, Cpu, Keyboard, Mouse, Plug, MousePointer2, Info, ArrowRightCircle } from 'lucide-react';
...
const DraggableCable = ({
  ...
  color, glowId, width = 4, isConnected, isDragging, dragPos, onPointerDown, label, opacity = 1
}: DraggableCableProps) => {
...
export default function App() {
  const [view, setView] = useState<'architecture' | 'assembly'>('architecture');
  ...
  if (view === 'assembly') {
    return <AssemblyView onBack={() => setView('architecture')} />;
  }
    ...
    <div className="min-h-screen bg-[#0c0c0e] text-[#d1d1d1] font-sans flex flex-col relative overflow-hidden">
      ...
      <header className="p-6 border-b border-[#1f1f23] flex flex-col xl:flex-row xl:items-center justify-between bg-[#0c0c0e] z-10 w-full gap-6">
        ...
        <div className="flex gap-3 text-[10px] font-mono tracking-tighter uppercase items-center flex-wrap">
          <button
            onClick={() => setConnections({ keyboard: false, mouse: false, display: false, pcPower: false, avrPower: false, monitorPower: false })}
            className="px-3 py-1.5 border border-red-500/50 text-red-400 hover:bg-red-500/10 transition-all mr-4 flex items-center gap-2"
            title="Disconnect all cables"
          >
            <Plug className="w-3 h-3 line-through" /> Disassemble
          </button>
          <button
          ...
          >
            <div className="flex items-start gap-4">
              <div className="p-3 rounded-full bg-[#3b82f6]/20">
                <ArrowRightCircle className="w-6 h-6 text-[#3b82f6]" />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-bold text-white mb-2 tracking-tight">External Setup Complete</h3>
                <p className="text-sm text-slate-300 leading-relaxed mb-4">
                  All foundational data and power links are physically established and verified. Proceed to internal component assembly.
                </p>
                <button 
